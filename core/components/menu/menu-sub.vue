...
export default {
	...
	computed: {
		...
		classes() {
			return [prefix + '-title', this.active && prefix + '-title-active']
		},
		tooltipUpward() {
			return findComponentUpward(this, 'zov-tooltip')
		},
		placement() {
			return this.menuComponent.currentThumbnail ||
				this.parentsMenuSub.length
				? 'right-start'
				: 'bottom'
		},
	...
	watch: {
		opened(val) {
			...
			val
				? menu.currentOpenNames.indexOf(name) === -1 &&
				  menu.currentOpenNames.push(name)
				: menu.$delete(
						menu.currentOpenNames,
						menu.currentOpenNames.indexOf(name)
				  )
	...
	methods: {
		...
		upDateOpened() {
			this.opened =
				this.menuComponent.currentOpenNames
					.join(',')
					.split(',')
					.indexOf(String(this.name)) !== -1
		},
		initFn() {
			this.tooltipUpward
