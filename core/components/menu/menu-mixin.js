...
export default {
    ...
    computed: {
        ...
        paddingStyle () {
            let num = this.isHorizontalMode || this.menuComponent.currentThumbnail
                ? this.parentsMenuGroup.length && !this.$parent.parentsMenuSub.length
                    ? 0
        ...
        checkHasIcon () {
            ...
            if (title) {
                for (let x in title.children) {
                    ...
                    if (el.getAttribute && el.getAttribute('thumbnail-icon') !== null) {
