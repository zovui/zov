<template>
    <div class="zov-time-spinner">
        ...
        >
            <ul
                class="zov-time-spinner-list-box"
            >
                <li
                    ...
                    :class="[
                        ...
                        {
                            'zov-time-spinner-list-cell-selected': item.text === currentTime[clockScaleIndex]
                        }
                    ]"
...
export default {
    ...
    data () {
        return {
            spinnerSteps: [1, 1, 1].map((one, i) => Math.abs(this.steps[i]) || one),
            currentTime: []
        }
    },
    computed: {
        ...
        hoursList () {
            ...
            for (let i = 0; i < 24; i += step) {
                ...
                if (this.disabledTimes.h.length && this.disabledTimes.h.indexOf(i) > -1) {
                    hour.disabled = true
        ...
        minutesList () {
            ...
            for (let i = 0; i < 60; i += step) {
                ...
                minute.text = i
                if (this.disabledTimes.m.length && this.disabledTimes.m.indexOf(i) > -1) {
        ...
        secondsList () {
            ...
            for (let i = 0; i < 60; i += step) {
                ...
                second.text = i
                if (this.disabledTimes.s.length && this.disabledTimes.s.indexOf(i) > -1) {
        ...
        value (val) {
            if (val.toString() !== this.currentTime.toString()) {
                this.currentTime = this.value.map((item, index) => {
                    ...
                    return y > this.spinnerSteps[index] / 2 ? item - y + this.spinnerSteps[index] : item - y
                })
            }
        },
        currentTime (val) {
